...
dir_out <- paste0(main_dir, "/ANALYSES/oct20_gender_PCA/vst/")
setwd(dir_out)
...
pl_gender + geom_point(size = 5)
if(output_save==TRUE){ dev.off() }

# => with labels
if(output_save==TRUE){ png(file = paste0("PCA_plot_", run_id, "_labelled.png")) }
pl_gender + geom_point(size = 5) + geom_text(label=pl_gender$data$name, check_overlap=F, nudge_x=5, nudge_y=0, size=3)
if(output_save==TRUE){ dev.off() }
